...
def scrape_website(base_url, document_id, conn, cursor, max_pages=20):
    ...
    with st.spinner(f"Collecting website data..."):
...
def main():
    ...
    st.set_page_config(layout="wide", page_title="AI Document Assistant")
    ...
    if st.session_state.data_source == "url":
        ...
        if url and st.button("Start Analysis"):
            ...
            with st.spinner("Analyzing website content. This may take a few minutes..."):
              
                pages_scraped = scrape_website(url, document_id, conn, cursor, max_pages)
                
                if pages_scraped > 0:
                    st.success(f"Website analysis complete. You can now ask questions about the content.")
        ...
        if uploaded_file and st.button("Start Analysis"):
            ...
            if text_content:
                ...
                st.success(f"Document analysis complete. You can now ask questions about {uploaded_file.name}")
    ...
    st.markdown("---")
    st.caption("AI Document Assistant powered by advanced language models")
